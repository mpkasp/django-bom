                    ...
                    if k in part_data and k+'_units' in part_data:
                        if part_data[k] and not part_data[k+'_units']:
                            ...
                            if is_valid_choice(part_data[k+'_units'], v):                 
                                ...
                                self.add_error(None, "'{0}' is an invalid choice of units for '{1}' for part in row {2}. Uploading of this part skipped.".format( part_data[k+'_units'], k+'_units', row_count))                           
                                skip = True
                                break
...
class PartRevisionForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not cleaned_data.get('description') and not cleaned_data.get('value'):
            validation_error = forms.ValidationError(
                                        ("Must specify a value and value units, or a description."),
                                        code='invalid')
                ...
                if not value and units:
                    self.add_error(key, 
                                    forms.ValidationError(
                                        ("Cannot specify {} without an accompanying value").format(units),
                                        code='invalid'))
                elif units and not units: 
                    self.add_error(key, 
                                    forms.ValidationError(
                                        ("Cannot specify value {} without an accompanying units".format(value)),
                                        code='invalid'))
